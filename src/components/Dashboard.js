...
const Dashboard = () => {
  ...
  useEffect(() => {
    if(user){
      try {
        axios
          .get(`http://event-management-app-backend-production.up.railway.app/${user._id}/events`)
          .then((response) => {
      ...
      try {
        axios
          .get(`http://event-management-app-backend-production.up.railway.app/task/userTasks/${user._id}/`)
          .then((response) => {
